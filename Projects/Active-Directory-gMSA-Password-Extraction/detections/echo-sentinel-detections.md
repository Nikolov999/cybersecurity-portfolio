# EchoSentinel Detection Notes — Active Directory gMSA Password Extraction Abuse
...
- Inventory and alert on who can read managed password blobs for every gMSA.
- Detect service-account interactive use, remote execution, or workstation-originated administration that deviates from baseline.
- Monitor 4624/4648/4672 patterns where gMSAs appear in unusual logon types or on non-service hosts.
- Correlate gMSA usage with DCSync indicators, remote admin tooling, and privileged group access.
- Flag changes that grant additional principals the right to retrieve managed passwords.
