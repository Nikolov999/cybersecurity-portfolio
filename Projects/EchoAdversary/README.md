...
### 1) Requirements
- Windows host (recommended) or lab-compatible system
- Python 3.11+ (or the packaged release if using binaries)
- A lab target VM (Windows endpoint/server) on your network
- Defender visibility tool (recommended: **EchoSentinel**)

### 2) Install
```powershell
git clone https://github.com/Nikolov999/EchoAdversary.git
cd EchoAdversary
python -m venv .venv
