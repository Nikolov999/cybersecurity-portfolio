# EchoSentinel Detection Notes — Active Directory Kerberos Ticket Abuse
...
- Correlate 4768, 4769, and 4624 events where a user rapidly shifts from password-based to ticket-heavy access from a single workstation.
- Alert on unusual WinRM logons using Kerberos from hosts that do not normally administer servers.
- Detect LSASS access or known credential-dumping tool behavior from Sysmon Event ID 10 or EDR telemetry.
- Flag anomalous DCSync behavior through 4662 with replication rights GUIDs, especially from non-DC systems.
- Hunt for ticket export or injection artifacts on endpoints, including abnormal Kerberos cache handling and suspicious child processes.
